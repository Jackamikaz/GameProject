...
//layout (location = 0) in vec3 gl_PositionIn[];
...
void main()  
{
	vec4 l_pos = vec4(uDirectionalLight, 0);
	int robust = 1;
	int zpass = 1;
...
//	};
	...
	if ( zpass == 0 ) { 
		...
		gl_Position = uProjMatrix*or_pos[0];  
		EmitVertex();  
		gl_Position = uProjMatrix*or_pos[1];  
		EmitVertex();  
		gl_Position = uProjMatrix*or_pos[2];  
		...
		gl_Position = uProjMatrix*v[0];  
		EmitVertex();  
		gl_Position = uProjMatrix*v[1];  
		EmitVertex();  
		gl_Position = uProjMatrix*v[2];  
		EmitVertex(); EndPrimitive();  
	}
